...
public partial class CMSModules_MMT_SmartSearch_SearchResults : CMSUserControl, ISearchFilterable, IUniPageable
{
    #region "Variables"
    ...
    protected void Search()
    {
        ...
        else
        {
            ...
            if (searchAllowed && (searchTextIsNotEmptyOrNotRequired || !SearchHelper.SearchOnlyWhenContentPresent))
            {
                ...
                // get ISearchDocument list from Lucene.NET
                var searchResults = MMTSearchHelper
                    .GetSearchDocuments(parameters);

                // select only pairs <ID, relevance>
                var searchPairs = searchResults
                    .Select((s, index) => new KeyValuePair<int, double>(Convert.ToInt32(s.Get("tripid")), Convert.ToDouble(s.Get("_score"))));

                // group pairs by trip ID
                var groupedTrips = searchPairs
                    .GroupBy(t => t.Key)
                    .Select(g => new KeyValuePair<int, double>(g.Key, g.Max(s => s.Value)));

                // update number of results for the pager
                parameters.NumberOfResults = groupedTrips.Count();

                // calculate on which page we currently are
                var startingPosition = Math.Max(parameters.StartingPosition, 0);
                groupedTrips = groupedTrips.Skip(startingPosition);
                var endingPosition = Math.Min(groupedTrips.Count(), parameters.DisplayResults);
                groupedTrips = groupedTrips.Take(endingPosition);
                
                // get data about trips from DB
                var results = DocumentHelper
                    .GetDocuments("MMT.Trip")
                    .Where("NodeID IN (" + string.Join(",", groupedTrips.Select(t => t.Key)) + ")")
                    .Execute();
                
                // add relevance to original set
                results.Tables[0].Columns.Add(new DataColumn("Relevance"));
                foreach (DataRow row in results.Tables[0].Rows)
                {
                    var trip = groupedTrips.SingleOrDefault(t => t.Key.ToString() == row["NodeID"].ToString()).Value;
                    row["Relevance"] = trip;
                }

                // sort based on relevance
                var view = results.Tables[0].DefaultView;
                view.Sort = "Relevance desc";
